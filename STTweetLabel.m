...
- (void)setupLabel {
    ...
    _attributesHashtag = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithRed:1.0/255.0 green:100.0/255.0 blue:255.0/255.0 alpha:1.0], NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:18.0]};
    _attributesLink = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithRed:1.0/255.0 green:100.0/255.0 blue:255.0/255.0 alpha:1.0], NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:14.0]};
    
    self.validProtocols = @[@"http", @"https"];
}

-(void)makeColorsLighter{
    //135-206-255
    _attributesHashtag = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithRed:135.0/255.0 green:206.0/255.0 blue:255.0/255.0 alpha:1.0], NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:18.0]};
    _attributesLink = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithRed:135.0/255.0 green:206.0/255.0 blue:255.0/255.0 alpha:1.0], NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:14.0]};}
...
- (void)updateText
{
    ...
    for (NSDictionary *dictionary in _rangesOfHotWords)  {
        ...
        if(_addLineSpacing == YES){
            // NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]init];
            NSMutableParagraphStyle *paragrahStyle = [[NSMutableParagraphStyle alloc] init];
            [paragrahStyle setLineSpacing:12];
...
@class FlickrHelper;
- (NSString*)getHref:(UIViewController *)controller {
    __block NSString *s = @"";
    [self setDetectionBlock:^(STTweetHotWord hotWord, NSString *string, NSString *protocol, NSRange range) {
        FlickrHelper *instance = [FlickrHelper new];
        [instance processClickableText:string viewController:controller];
        s = string;
